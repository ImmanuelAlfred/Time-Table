    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.time-tables.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <div class="needsclick dropzone {{ $errors->has('time_table') ? 'is-invalid' : '' }}" id="time_table-dropzone">
                </div>
                @if($errors->has('time_table'))
                    <span class="text-danger">{{ $errors->first('time_table') }}</span>
                @endif
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('department_name') ? 'is-invalid' : '' }}" name="department_name_id" id="department_name_id" required>
                    @foreach($department_names as $id => $entry)
                        <option value="{{ $id }}" {{ old('department_name_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
...
Dropzone.options.timeTableDropzone = {
    url: '{{ route('admin.time-tables.storeMedia') }}',
    maxFilesize: 20, // MB
    addRemoveLinks: true,
    headers: {
